...
def search(query: str, paths: list, frecency: dict = None) -> list:
    ...
    # Optimized: Using list comprehensions with the walrus operator avoids manual loop overhead
    # and `.append()` calls, which provides significant performance improvements in Python.
    if is_tuple:
        if frecency:
            scored = [
