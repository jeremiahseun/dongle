...
## 2024-05-21 - Avoid pathlib.Path allocations in IO-bound loops
**Learning:** Instantiating `pathlib.Path` objects and calling `.relative_to()` or accessing `.parts` inside high-frequency IO-bound loops like `os.walk` introduces severe overhead (nearly 4x slower in synthetic benchmarks). The overhead stems from under-the-hood path parsing, tuple allocations, and method calls.
**Action:** When walking directory trees where performance is paramount (like caching thousands of directories), replace `pathlib` manipulations with direct string slicing and standard `os.path` operations.
