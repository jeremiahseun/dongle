...
## 2024-05-21 - Avoid pathlib in IO-bound Hot Loops
**Learning:** Instantiating `pathlib.Path` objects and calling `.relative_to()` inside a tight IO-bound loop like `os.walk` (which processes thousands of directories) creates significant object allocation overhead, making the loop extremely slow in Python.
**Action:** When walking large directory trees or doing bulk path processing, use C-optimized string slicing (`curr_root[root_len:]`) and native string methods to compute relative paths instead of relying on `pathlib`. This pattern provides massive performance boosts (e.g., ~6x speedup) for directory scanners.
