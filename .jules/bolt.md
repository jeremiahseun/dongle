...
## 2024-05-21 - Avoid pathlib.Path overhead in IO-bound loops
**Learning:** Instantiating `pathlib.Path` objects and calling `.relative_to()` inside a hot IO-bound loop like `os.walk()` traversing thousands of directories creates significant overhead due to object creation and path parsing.
**Action:** When walking large directory trees where speed is critical, use standard `os.path` and direct string slicing (e.g., `curr_root[len_of_base:]`) to calculate relative paths instead of `pathlib.Path`.
